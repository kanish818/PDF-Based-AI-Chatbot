"""
Evaluation runner for DocuMind retrieval and answer quality.
...
NO_ANSWER_PATTERNS = (
    ...
    "unable to answer from the provided document",
)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run retrieval/answer evaluation for DocuMind.")
    ...
    parser.add_argument("--top-k", type=int, default=5, help="Number of retrieved chunks to score. Default: 5.")
    parser.add_argument(
        "--with-llm",
        action="store_true",
        help="Also run the Groq answer generation step and score answer/citation behavior.",
    )
    parser.add_argument(
        "--output-dir",
...
def _extract_citations(answer: str) -> List[Tuple[str, int]]:
    ...
    for match in re.finditer(r"\[([^,\]]+),\s*Page\s+(\d+)\]", answer):
        filename = _normalise_filename(match.group(1))
        page_num = int(match.group(2))
        citations.append((filename, page_num))
    return citations
...
def _phrase_match_fraction(answer: Optional[str], phrases: List[str]) -> Optional[float]:
    ...
    matched = sum(1 for phrase in phrases if phrase.lower() in lowered_answer)
...
def _no_answer_correct(answer: Optional[str], should_answer: bool) -> Optional[bool]:
    ...
    detected_no_answer = any(pattern in lowered for pattern in NO_ANSWER_PATTERNS)
...
def _build_summary(results: List[CaseResult], with_llm: bool) -> Dict[str, Any]:
    ...
    phrase_scores = [result.phrase_match_fraction for result in valid_results if result.phrase_match_fraction is not None]
    no_answer_scores = [result.no_answer_correct for result in no_answer_cases if result.no_answer_correct is not None]
    ...
    summary = {
        ...
        "recall_at_1": round(_mean([1.0 if result.recall_at_1 else 0.0 for result in answerable]), 4),
        "recall_at_3": round(_mean([1.0 if result.recall_at_3 else 0.0 for result in answerable]), 4),
        "recall_at_5": round(_mean([1.0 if result.recall_at_5 else 0.0 for result in answerable]), 4),
        "mrr": round(_mean([result.reciprocal_rank for result in answerable]), 4),
    }

    if with_llm:
        summary.update(
            {
                "citation_hit_rate": round(_mean([1.0 if result.citation_hit else 0.0 for result in answerable]), 4),
