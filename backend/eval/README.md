...
- `document_filenames`: Exact uploaded PDF filenames to search within.
...
- `should_answer`: `true` when the PDF contains the answer, `false` when the assistant should refuse.
...
1. Upload your chosen benchmark PDFs with the target user account.
2. Prepare a 50-question dataset JSON by copying the example file.
3. Run the evaluation once in retrieval-only mode.
4. Fix retrieval issues.
5. Run again with `--with-llm` to score answer quality and no-answer behavior.
6. Submit the latest Markdown report as evidence of retrieval quality.
